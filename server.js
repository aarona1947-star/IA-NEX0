...
function setUsers(u) {
  try { fs.writeFileSync(DB, JSON.stringify(u, null, 2)); return true; }
...
app.post('/auth/register', async function(req, res) {
  try {
    ...
    res.status(500).json({ error: 'Error en el servidor. Intenta de nuevo.' });
  }
});
...
app.post('/auth/login', async function(req, res) {
  try {
    ...
    if (!user)
      return res.status(401).json({ error: 'No existe cuenta con ese email. Crea una cuenta primero.' });
...
function buildPrompt(agentId) {
  ...
  return (
    ...
    '- Responde CUALQUIER pregunta: matematicas, ciencia, historia, codigo, filosofia, cocina, viajes, deportes, noticias, arte\n' +
    ...
    '- Para matematicas: da el resultado exacto y correcto. 1+9=10. 15*8=120. Nunca cambies los numeros\n' +
    '- Para codigo: da el fragmento que funcione, nada mas\n' +
    '- Para preguntas simples: respuesta corta y directa\n' +
    '- Para preguntas complejas: respuesta completa y bien estructurada\n' +
    '- Si hay archivo adjunto: analizalo de forma util y directa\n' +
    '- Se natural, como un amigo muy inteligente que sabe de todo'
  );
}
...
function llamarGemini(history, text, agentId, fileData, cb) {
  ...
  if (!key || key.startsWith('PEGA_'))
    return cb(new Error('Falta configurar GEMINI_KEY. Pidele al admin que la configure.'));
...
app.post('/api/imagen', auth, function(req, res) {
  ...
  res.json({
    imageUrl: 'https://image.pollinations.ai/prompt/' +
      ...
      '?width=768&height=768&seed=' + seed + '&nologo=true&nofeed=true'
  });
});
