...
function buildPrompt(agentId, userCtx) {
  ...
  var base = cfg.prompt + '\n\nREGLAS ABSOLUTAS:\n' +
    ...
    '- Si es simple, responde en 2-3 oraciones brillantes. Si es compleja, despliega profundidad.\n' +
    '- En temas empresariales o financieros: máxima precisión, nunca inventes cifras, distingue dato de estimación.';
  // Contexto personalizado del usuario / empresa
  if (userCtx && userCtx.trim()) {
    base += '\n\nCONTEXTO DEL USUARIO (tenlo en cuenta siempre):\n' + userCtx.trim();
  }
  return base;
}
...
// ═══════════════════════════════════════════
function llamarGemini(history, text, agentId, files, userCtx, cb) {
  ...
  var bodyConfig = {
    system_instruction: { parts: [{ text: buildPrompt(agentId, userCtx) }] },
    contents: hist,
...
['general', 'sabio', 'saber', 'codigo'].forEach(function(agente) {
  app.post('/api/' + agente, auth, checkPlan, function(req, res) {
    ...
    llamarGemini(req.body.history || [], req.body.text || '', agente, files, userCtx,
      function(err, reply) {
...
app.post('/api/creativo', auth, checkPlan, function(req, res) {
  ...
  var userCtx = (req.userDoc && req.userDoc.contexto) || '';
  var files = req.body.files || req.body.file || null;
  llamarGemini(req.body.history || [], text, 'creativo', files, userCtx,
    function(err, reply) {
      if (err) return res.status(500).json({ error: err.message });
      respond(req, res, reply);
    }
...
app.listen(PORT, function() {
  ...
  console.log('\n  IA-NEXO v14 — Single File + Web Search + Multi-archivo');
